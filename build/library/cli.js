...
program
    ...
    const file = wick_component_js_or_html, dir = cmd.path ? path.resolve(cmd.path) : process.cwd(), name = cmd.output_name ? cmd.output_name : "index.html", maininbodyid = cmd.placeRootInBody || "";
    let head = cmd.head ? path.resolve(cmd.head) : "", body = cmd.body ? path.resolve(cmd.body) : "";
    if (head)
        try {
    ...
    //Stamp Component
    const root = await wick.stamp(wick(file)), options = {
        head,
        body,
