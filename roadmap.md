...
- CSS templating
- Server JSON API tooling
- Simplify, Optimize code base
- Form Validation using Schemes

### Features Introduced
- Complex HTML tempalate components that handle bindings to forms, CSS, attributes, strings
...
### v0.3.2b - *current* - API Documentation Started.
...
- Changed template binding point syntax from `[`\*\*\*`]`to `((`\*\*\*\*`))` to prevent confusion with non template text. Also bind points don't need to be a descendant in a `<w>` tag. (*auto-tap insertion*).
