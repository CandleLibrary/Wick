...
export class PluginStore {
    ...
    async runClientPlugin(presets:PresetOptions, type:string, selector:string, ...args:any[]){
        ...
        if(plugin)try{
            return await plugin.clientHandler(presets, ...args);
        }catch(e){
            console.log(e)
        }
    ...
    async runServerPlugin(presets:PresetOptions, type:string, selector:string, ...args:any[]){
        const plugin = this.getPlugin(type, selector);
        if(plugin)try{
            return await plugin.clientHandler(presets, ...args);
        }catch(e){
...
export function addPlugin(presets:PresetOptions, plugin:PluginUnion){

    const store = presets.plugins;
